<?php
    ...
    if (isset($_POST['btnsubmit'])) {
        ...
        if ((!empty($amount) || ($amount >= $minamount)) && !empty($namarek) && !empty($norek)) {
        	if ($parenttrans = cekparenttrans($idafiliasi,$idmember,$account)) {
	        	...
	        	$q = "INSERT INTO tb_trans(idmember,idafiliasi,parenttrans,amount,iphost,tgl,idbank,namarek,norek,idstatus,idstatusrmi,idstatussettlement,idtranskode,lastupdate) 
	            		VALUES('$idmember','$idafiliasi','$parenttrans','$amount','$iphost','$tgl','$idbank','$namarek','$norek','1','1','1','2','$lastupdate')";
				...
				$qs = "SELECT * FROM tb_user WHERE idlevel='2'"; //khusus rmi
				...
				$mail->isHTML(true);
				$stremailsubject = getemailsubject(5,$webtitle);
				$e_body = getemailisi(5);
				...
				//$mail->Subject = '[Deposit] '.$webtitle;
				//$mail->Body = "Hallo,\r\n\r\nAda deposit baru, detailnya sebagai berikut:\r\n\r\nTanggal: $e_tgl\r\nProduk: $e_produk\r\nJumlah: $e_jumlah\r\n\r\nBest Regards\r\n\r\nTeam $webtitle";
				$mail->send();
	            $strstatus = "Deposit telah ditambahkan.";
			} else {
				$strstatus = "Data akun produk dengan produk yang dipilih tidak ada/statusnya belum selesai.";
				$act = "adx";
			}
        } else $strstatus = "Masukkan jumlah deposit atau jumlah deposit kurang atau nama rekening/nomor rekening kosong.";
    }
    switch($act) {
        case 'adx':
            $sedangedit = true;
...
<?php
    if ($sedangedit) {
    	include("frmtransdepo.inc.php");
    } else {
?>
<h3>List Deposit</h3>
<a href="member.php?m=<?=$m?>&a=adx" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-plus icon-white"></i> Deposit</a>
<p></p>
...
<?php
	$q = "SELECT a.*,b.judul,c.status,d.nama AS bank,e.account FROM tb_trans a
			...
			WHERE a.idmember='$idmember' AND a.idtranskode='2'
			ORDER
			 BY a.idtrans DESC";
    if (!$result = $conn->query($q)) die ($conn->error);
