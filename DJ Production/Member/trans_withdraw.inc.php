        ...
        if (!empty($amount) && !empty($account)) {
        	if ($parenttrans = cekparenttrans($idafiliasi,$idmember,$account)) {
	        	...
	        	$q = "INSERT INTO tb_trans(idmember,idafiliasi,parenttrans,amount,iphost,tgl,idbank,namarek,norek,idstatus,idstatusrmi,idstatussettlement,idtranskode,lastupdate) 
	            		VALUES('$idmember','$idafiliasi','$parenttrans','$amount','$iphost','$tgl','$idbank','$namarek','$norek','1','1','1','3','$lastupdate')";
				...
				$mail->isHTML(true);
				$stremailsubject = getemailsubject(6,$webtitle);
				$e_body = getemailisi(6);
...
<?php
    if ($sedangedit) {
    	include("frmtranswithdraw.inc.php");
    } else {
?>
<h3>List Withdraw</h3>
<a href="member.php?m=<?=$m?>&a=adx" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-plus icon-white"></i> Withdraw</a>
<p></p>
...
<?php
	$q = "SELECT a.*,b.judul,c.status,d.nama AS bank,e.account FROM tb_trans a
			...
			WHERE a.idmember='$idmember' AND a.idtranskode='3'
