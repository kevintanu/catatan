<?php
	if (isset($_POST['btndelall'])) {
		if ($_SESSION['islevel'] == '1') {
			$q = "DELETE FROM tb_trans";
			if (!$result = $conn->query($q)) die ($conn->error);
			$q = "DELETE FROM tb_trans_account";
			if (!$result = $conn->query($q)) die ($conn->error);
			$q = "DELETE FROM tb_trans_transfer";
			if (!$result = $conn->query($q)) die ($conn->error);
			$strstatus = "Semua data transaksi telah dihapus";
		}
	}
	if (isset($_POST['btndelcb'])) {
		$cbd = $_POST['cbd'];
		$c_cbd = count($cbd)-1;
		if ($c_cbd > -1) {
			for($i=0;$i<=$c_cbd;$i++) {
				$q = "SELECT idtrans, idtranskode FROM tb_trans WHERE idtrans='$cbd[$i]'";
				echo $q."<Br>";
				if (!$result = $conn->query($q)) die ($conn->error);
				$r = $result->fetch_assoc();
				switch($r[idtranskode]) {
					case "1";
						$qd = "DELETE FROM tb_trans_account WHERE idtrans='$r[idtrans]'";
						if (!$resultd = $conn->query($qd)) die ($conn->error);
						$qd = "DELETE FROM tb_trans WHERE idtrans='$r[idtrans]'";
						if (!$resultd = $conn->query($qd)) die ($conn->error);
						break;
					case "2":
					case "3":
						...
						break;
					case "4":
						$qd = "DELETE FROM tb_trans_transfer WHERE idtrans='$r[idtrans]'";
						if (!$resultd = $conn->query($qd)) die ($conn->error);
						$qd = "DELETE FROM tb_trans WHERE idtrans='$r[idtrans]'";
						if (!$resultd = $conn->query($qd)) die ($conn->error);
						break;
			...
			$strstatus = "Sebanyak " . $c_cbd+1 . " data telah dihapus.";
		} else $strstatus = "Pilih dahulu transaksi yang akan dihapus.";
	}	
	if (!empty($_GET['sea'])) {
		$sea = $_GET['sea'];
	...
	if (!$result = $conn->query($q)) die ($conn->error);
	echo pstrstatus($strstatus);
?>
<h3>List Transaksi Admin</h3>
<P></P>
...
<form role="form" action="main.php?m=<?=$m?>" method=post name=f2>
<p></p>
...
<tbody>
	<tr>
		<Td colspan=9 align=center><input type=submit name="btndelcb" class="btn btn-warning" value="Hapus Pilihan" onclick="return confirm('Anda yakin akan menghapus tranksasi yang dipilih?')"> <input type=submit name="btndelall" class="btn btn-danger" value="Hapus Semua" onclick="return confirm('Anda yakin akan menghapus semua transaksi yang ada? Jangan lupa untuk melakukan backup list transaksi terlebih dahulu sebelum melakukan penghapusan. Yakin ingin menghapus?')"></Td>
	</tr>
<?php
...
?>    
<tr>
	<td><input type=checkbox name=cbd[] value="<?=$r[idtrans]?>"></td>
